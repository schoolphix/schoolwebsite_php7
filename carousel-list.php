...
getDeleteConfirmation(1,"Selected carousel(s) will be deleted. Continue?");
            ...
            <div class="col-md-9 page-content">
               ...
               <? callErrorMessage();?>
                ...
                <span style="float:right">
                	<a href="carousel-new"><b>Add Carousel Picture</b></a>
                </span>
                </h4>
...
<?
  	$conn = connectAppDB(); 
		...
		if(isset($_GET['offset']) && is_numeric($_GET['offset']) && $_GET['offset'] >= 0){
			...
			$url = "?";      
			$totalno = $conn->query("SELECT COUNT(*) FROM carousel");
			list($total_rows) = $totalno->fetch_array();
			//to make pagination
			$statement = "`carousel`";
		 	$myquery = "SELECT * FROM carousel ORDER BY id ASC LIMIT ".$offset."," .PAGE_LIMIT;
			$displayMsg = "Carousel";
			$nullMsg = "No carousel picture was found.";	
    ...
    <table width="100%" border="0" align="center" class="tablet4" cellspacing="0">
  			<tr>
            	<td align="left" colspan="2">
                	<input type='hidden' name='action' value='delete-carousel'/>
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="mytable" id="datalist">
 	...
 	<? while(@$record = $list->fetch_assoc()){?>
 		<tr <?=($i%2?"class='light'":"class='dark'")?>>
            ...
            <td align="center"><a class="modhomelinks" href='carousel-details?id=<?=$record["id"]?>' title = 'click to view'>Edit</a></td>       
		</tr>
		<? $i++;?>
...
echo "<br><br>";
navigation ($offset,"carousel-list",$total_rows,$url);?>				
