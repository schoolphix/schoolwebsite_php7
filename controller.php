...
function login(){
	try{
		$conn = connectAppDB();// COnnect to Database
		$nextURL = "login";
		...
		$result = $conn->query("SELECT * FROM adminuser WHERE userid='".$email."' AND pass='".sha1($password)."' LIMIT 1");
	...
	try{
		...
		if($_REQUEST['oldpass']==""||$_REQUEST['newpass']==""||$_REQUEST['newpasssecond']==""){
			...
			if ($Query->num_rows>0){
				...
				$updatestudent = $conn->query("UPDATE adminuser SET pass = '".sha1($newpass)."' 
				where id = '".$u->id."'");
				if ($updatestudent != ""){
...
function settingNew(){
	try{	
		...
		if($_REQUEST['caption']==""){
			...
			$chkcat = $conn->query("SELECT * FROM settings WHERE caption ='".$caption."'");
			if($chkcat->num_rows > 0){

				$update = $conn->query("UPDATE settings SET content='".$content."' WHERE caption='".$caption."' LIMIT 1");
				...
				$insertcat = $conn->query("INSERT INTO settings (caption, content, datecreated, settingtype) VALUES
		...
		if($_FILES['photo']['name'] == ""){		
			...
			if($upload['upload']=="true"){
				...
				if($chkcat->num_rows > 0){
					...
					$chkimg = $chkcat->fetch_array();
					if($chkimg['content'] != ""){			
						@unlink($chkimg['content']);
					}

					$update = $conn->query("UPDATE settings SET content='".$upload['picturename']."' WHERE caption='".$caption."' LIMIT 1");
					if ($update != ""){
						$success="true";			
					}
	
				}else{//New category name
					$insert = $conn->query("INSERT INTO settings (caption, content, datecreated, settingtype) VALUES
...
function updateNews(){
	try{
		...
		if(trim($_REQUEST['caption'])==""||trim($_REQUEST['description'])==""){
			...
			$update = $conn->query("UPDATE adminupdate
			...
			if($update != ""){//
				...
				if($_FILES['photo']['name'] != ""){		
					...
					$chkcat = $conn->query("SELECT upload 
...
function updatePageDetails(){
	...
	//caption, picture_url, active_status, category link_location, url_content, content, created_on
		...
		if(trim($_REQUEST['caption'])==""||trim($_REQUEST['url_content'])==""||trim($_REQUEST['category'])==""){
			...
			$update = $conn->query("UPDATE morepages 
			SET caption='".$caption."',
			...
			if($update != ""){//
				...
				if($_FILES['photo']['name']!=""){
					...
					"'";	 	
					$qupld = $conn->query($qupload);	
					$chkimg = $qupld->fetch_array();
					...
					if($upload['upload']=="true"){
						$update = $conn->query("UPDATE morepages SET picture_url='".$upload['picturename']."' WHERE id='".$id."' LIMIT 1");
					}
...
function saveSettings(){
	...
	try{	
		...
		foreach($_POST as $key => $value){
			$update = $conn->query("UPDATE settings
...
function contactUs(){
	try{
		...
		if(trim($_REQUEST['clientname']) == "" || trim($_REQUEST['message']) == ""){
			...
			$insert = $conn->query("INSERT INTO contactus (email, contactsubject, message, names, datesent, phoneno) VALUES
		...
		if ($count == 0){
			...
			for($e =0; $e <=$count; $e++){
				...
				if($table=="picfolder"){
					...
					($_REQUEST['messages'][$e])."'";	 	
					$qupld = $conn->query($qupload);	
					while(@$chkimg = $qupld->fetch_assoc()){						
						if($chkimg['imagedirectory'] != ""){			
							@unlink($chkimg['imagedirectory']);
						}//end of if
	
					}// end of while loop
					$conn->query("DELETE FROM gallerypicures WHERE folderid = '".getCleanString($_REQUEST['messages'][$e])."'");
				}
				...
				$deletequery =$conn->query("DELETE FROM ".$table." WHERE ".$field." = '".getCleanString($_REQUEST['messages'][$e])."'"); 
	  	...
	  	if($_REQUEST['albumid']==""){
			...
			for($e =0; $e < $count; $e++){
				if($_FILES['photo']['name'][$e]!=""){
					...
					$insert = $conn->query("INSERT INTO gallerypicures (folderid,dateuploaded) 
						values
					('".$albumid."','".getDateTime()."')");
					$id = (int) $conn->insert_id();
				...
				$updateupload = $conn->query("UPDATE
...
function addTestimony(){
	try{	
		...
		if($_FILES['photo']['name'] != ""){		
			$upload = justUpload($fileformat,$directory,$width,$height,"logo");
			$picfile = $upload['picturename'];
		}
					
		$insert = $conn->query("INSERT INTO 
		testimonies (name, testimony, picture_url,
		 dateAdded, status) VALUES
		('".$name."','".$testimony."','".$picfile."', '".date("Y-m-d H:i:s")."', 'ACTIVE')");
	...
	try{
		...
		$fileformat = array('pdf','doc','docx','xls','xlsx');
			...
			if($chkcat->num_rows > 0){
				...
				$query = $conn->query("INSERT INTO uploads (uploadtype, uploadedby, uploadedon) 
					values
...
function newPhotoAlbum(){
	try{	
		...
		if($_REQUEST['caption']==""){
			...
			$chkcat = $conn->query("SELECT * FROM picfolder WHERE caption ='".$caption."'");
			if($chkcat->num_rows > 0){//Category names are unique, but books can have same name or titles etc
				$_SESSION['error'][] = 'The photo album entered has already been created before.';	
				$_SESSION['alertstyle'] = "alert";
			}else{//New category name
				$insertcat = $conn->query("INSERT INTO picfolder (caption, datecreated, createdby, description) VALUES
...
function newJob(){
	try{	
		...
		if($_REQUEST['caption']=="" || $_REQUEST['description']=="" || $_REQUEST['requirements']==""){
			...
			$chkcat = $conn->query("SELECT * FROM careers WHERE title ='".$caption."'");
			if($chkcat->num_rows > 0){//Category names are unique, but books can have same name or titles etc
				$_SESSION['error'][] = 'The job title entered has already been created before.';	
				$_SESSION['alertstyle'] = "alert";
			}else{//New category name
				$insertcat = $conn->query("INSERT INTO careers (title, description, requirements, createdby, createdon, status) VALUES
...
function updateCarousel(){
	try{	
		...
		if($_REQUEST['caption']==""){
			...
			$update = $conn->query("UPDATE carousel
			SET caption='".$caption."', 
			active_status='".$active_status."' 
			WHERE id='".$id."'");
			...
			if ($update != ""){
				...
				if($_FILES['photo']['name'] != ""){		
					...
					$chkcat = $conn->query("SELECT picture_url 
...
function updateTestimony(){
	try{	
		...
		if($_REQUEST['testimony']=="" || $_REQUEST['name']==""){
			...
			$update = $conn->query("UPDATE testimonies
...
function updatePhotoAlbum(){
	try{	
		...
		if($_REQUEST['caption']==""){
			...
			$chkcat = $conn->query("SELECT * FROM picfolder WHERE caption ='".$caption."' AND id!='".$id."'");
			if(mysql_num_rows($chkcat) > 0){//Category names are unique, but books can have same name or titles etc
				$_SESSION['error'][] = 'The photo album entered has already been created before.';	
				$_SESSION['alertstyle'] = "alert";
			}else{//New category name
				$update = $conn->query("UPDATE picfolder SET caption='".$caption."', description='".$description."' WHERE id='".$id."'");
...
function updateJob(){
	try{	
		...
		if($_REQUEST['caption']=="" || $_REQUEST['description']=="" || $_REQUEST['requirements']==""){
			...
			$chkcat = $conn->query("SELECT * FROM careers WHERE title ='".$caption."' AND id!='".$id."'");
			if($chkcat->num_rows > 0){//Category names are unique, but books can have same name or titles etc
				$_SESSION['error'][] = 'The job entered has already been created before.';	
				$_SESSION['alertstyle'] = "alert";
			}else{//New category name
				$update = $conn->query("UPDATE careers SET title='".$caption."', description='".$description."', requirements='".$requirements."', status='".$status."' WHERE id='".$id."'");
...
function newsLetterNew(){
	try{	
		...
		if($_REQUEST['useremail']==""){
			...
			$chkcat = $conn->query("SELECT id FROM
...
function socialMedia(){
	...
	try{	
		...
		if(trim($_REQUEST['caption'])==""||trim($_REQUEST['medialink'])==""){
			...
			$chkcat = $conn->query("SELECT * FROM socialmedia WHERE caption ='".$caption."'");
			if($chkcat->num_rows > 0){

				$update = $conn->query("UPDATE socialmedia SET medialink='".$medialink."' WHERE caption='".$caption."' LIMIT 1");
				...
				$insert = $conn->query("INSERT INTO socialmedia (caption, medialink, dateadded) VALUES
...
function sendNewsletter(){
	try{	
		...
		if($insert != ""){
			...
			$list = $conn->query($myquery);	
			$countlist = @$list->num_rows;
			if($countlist>0){
				while(@$email = $list->fetch_assoc()){
