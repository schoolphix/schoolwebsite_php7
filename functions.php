...
function connectAppDB() {
    ...
    $dbname = 'schweb_db';//DB/Schema
	$connect = new mysqli($dbhost, $dbuser, $dbpassword, $dbname);//using @ to suppress any possible error that may arise from using this function
...
function getCleanString($input){
	$conn = connectAppDB();
 	$clean = mysqli_real_escape_string($conn,trim($input));
	return $clean;
}
...
function getPictureCount($fid){
	...
	//echo $query;
	$result = $conn->query($query);
	list($piccount) = $result = $result->fetch_array();
	return $piccount;
}
...
function getTestimonyById($fid){
	...
	if($result->num_rows($result) == 0){
		...
		$myresult = $result->fetch_array($result);
	}		
