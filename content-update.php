...
if(isset($_POST['action']) && $_POST['action'] == "update-content"){//when users clicks on the submit button
	...
	try{
		...
		if($content==""){
			...
			adminupdate (caption, content, 
			upload, userid, updatetype, 
			uploadedon) VALUES 
			('".$caption."','".$content."',
			'".$upload['picturename']."',
			'".$u->id."','".$option."',
...
<div id="content">
        ...
        <div class="row sidebar-page">
            ...
            <div class="col-md-9 page-content">
               ...
               <? callErrorMessage();?>
                ...
                <!-- Classic Heading -->
                <h4 class="classic-title"><span><?="Add $title"?></span>
                	<span style="float:right">
                		<a href="content-list"><b>See All <?=ucwords(strtolower($option))?></b></a>
                	</span>
    ...
    <? 
   		...
   		if($option=="elearning"){
			$conn = connectAppDB();
			$fetchdata = $conn->query("SELECT * FROM adminupdate WHERE updatetype = '".$option."' 
			AND updatemode = '".$mode."' LIMIT 1");
			$CNT = $fetchdata->fetch_array();
...
<input type="checkbox" style="cursor:pointer" name="remember" onClick="" id="check_resize"/>
 ...
 <?	}?>
    ...
    <? }?>
		  ...
		  <?=($option=="video"?"<b>YouTube Link Src</b>":"")?>
